 ...
 */
    ...
    mapping(address=>uint) public referrerRewardPointsEarned;//NEED TO optimize it 
    //mapping(address=>bytes32) refererReferrals;
    //mapping(address=>bytes32) refereeReferrals;
    mapping(address=>mapping(address=>uint)) public referrerToReferee;
    mapping(address=>mapping(uint=>bytes32)) public refereeReferrals;

    event Referral(address _referrer, address _referee, bytes32 _referralId);
}
